...
class EditGameScreen extends React.PureComponent<IProps, IState> {
	...
	constructor(props: IProps) {
		super(props);
		const sport: ISport | undefined = this.props.navigation!.getParam(
			'sport'
		);
		const gameEditData:
			| IGameEditData
			| undefined = this.props.navigation!.getParam('gameEditData');
		if (!sport && !gameEditData) {
			throw new Error(
				'sport or gameEditData must be provided in editGame screen '
			);
		}
	...
	private getDateStartRestrictions() {
		...
		const restrictions: IRestrictions = {
			...
			max:
				dateEnd === 0
					? Math.max(dateEnd, MAX_PEOPLE_COUNT)
					: dateEnd - 1
		};
		return restrictions;
	}
							...
							<SectionItemWithModal
								...
								modal={api => (
									<EditPeopleCount
										...
										onSave={
											this.onChangeCountminParticipants
										}
							...
							<SectionItemWithModal
								...
								modal={api => (
									<EditPeopleCount
										...
										onSave={
											this.onChangeCountmaxParticipants
										}
...
function getNewGameVariablesFromState(
	state: IState,
	authorId: string,
	sportId: number
) {
