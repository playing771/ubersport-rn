...
import { Animated, StyleSheet, Easing, ViewStyle, LayoutChangeEvent } from 'react-native';
...
const Expand = class extends React.Component<IExpandProps, IExpandState> {
  ...
  constructor(props: IExpandProps) {
    ...
    this.state =
      this.props.direction === ExpandDirection.Horizontal
        ? {
            width: this.props.animatedValue || new Animated.Value(this.props.minWidth || 0),
            maxWidth: 0,
  ...
  private getParamToAnimate() {
    return this.props.direction === ExpandDirection.Horizontal ? 'width' : 'height';
  }

  private getMinParamToAnimate() {
    return this.props.direction === ExpandDirection.Horizontal ? 'minWidth' : 'minHeight';
  }

  private getMaxParamToAnimate() {
    return this.props.direction === ExpandDirection.Horizontal ? 'maxWidth' : 'maxHeight';
  }
  ...
  private setMaxWidthHeight = (event: LayoutChangeEvent) => {
    const layoutHeightWidth = event.nativeEvent.layout[this.getParamToAnimate()];
    if (this.props.direction === ExpandDirection.Horizontal) {
      this.setState({
        maxWidth: Math.max(this.props.maxWidth || layoutHeightWidth, layoutHeightWidth),
      });
    } else {
      this.setState({
        maxHeight: Math.max(this.props.maxHeight || layoutHeightWidth, layoutHeightWidth),
      });
    }
  };
  ...
  render(): JSX.Element {
    ...
    return (
      <View style={[style]}>
        <Animated.View
          style={[styles.menuStyle, { [this.getParamToAnimate()]: value }, containerStyle]}
        >
